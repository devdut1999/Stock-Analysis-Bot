...
import { getAgentDefinitions, getAgentsByRole } from '../config/agents.js';
 ...
 */
async function executeAgent(agent, context, marketData) {
 ...
 */
function formatMarketDataContext(data) {
    const lines = [];
    ...
    if (data.fundamentals && Object.keys(data.fundamentals).length > 0) {
        ...
        if (data.fundamentals.peRatio)
            lines.push(`P/E Ratio: ${data.fundamentals.peRatio.toFixed(2)}`);
        if (data.fundamentals.pegRatio)
            lines.push(`PEG Ratio: ${data.fundamentals.pegRatio.toFixed(2)}`);
        if (data.fundamentals.priceToBook)
            lines.push(`Price-to-Book: ${data.fundamentals.priceToBook.toFixed(2)}`);
        if (data.fundamentals.debtToEquity)
            lines.push(`Debt-to-Equity: ${data.fundamentals.debtToEquity.toFixed(2)}`);
        if (data.fundamentals.roe)
            lines.push(`ROE: ${data.fundamentals.roe.toFixed(2)}%`);
        if (data.fundamentals.roce)
            lines.push(`ROCE: ${data.fundamentals.roce.toFixed(2)}%`);
        if (data.fundamentals.earningsPerShare)
            lines.push(`EPS: ${data.fundamentals.earningsPerShare.toFixed(2)}`);
        if (data.fundamentals.dividendYield)
            lines.push(`Dividend Yield: ${data.fundamentals.dividendYield.toFixed(2)}%`);
        if (data.fundamentals.revenueGrowth)
            lines.push(`Revenue Growth: ${data.fundamentals.revenueGrowth.toFixed(2)}%`);
        if (data.fundamentals.profitMargin)
            lines.push(`Profit Margin: ${data.fundamentals.profitMargin.toFixed(2)}%`);
        if (data.fundamentals.operatingMargin)
            lines.push(`Operating Margin: ${data.fundamentals.operatingMargin.toFixed(2)}%`);
        lines.push('');
    }
    ...
    if (data.technicals.rsi !== null) {
        ...
        if (data.technicals.movingAverages) {
            ...
            if (data.technicals.movingAverages.sma20)
                lines.push(`  SMA-20: ${data.technicals.movingAverages.sma20.toFixed(2)}`);
            if (data.technicals.movingAverages.sma50)
                lines.push(`  SMA-50: ${data.technicals.movingAverages.sma50.toFixed(2)}`);
            if (data.technicals.movingAverages.ema12)
                lines.push(`  EMA-12: ${data.technicals.movingAverages.ema12.toFixed(2)}`);
            if (data.technicals.movingAverages.ema26)
                lines.push(`  EMA-26: ${data.technicals.movingAverages.ema26.toFixed(2)}`);
        }
 ...
 */
export async function orchestrateAnalysis(marketData) {
    const startTime = Date.now();
    ...
    const technicalPromises = technicalAgents.map(agent => executeAgent(agent, marketContext, marketData));
    console.log(`[Orchestrator] Phase 2: Fundamental Analysis Panel (${fundamentalAgents.length} agents)`);
    const fundamentalPromises = fundamentalAgents.map(agent => executeAgent(agent, marketContext, marketData));
    console.log(`[Orchestrator] Phase 3: Trading & Risk Panel (${tradingAgents.length} agents)`);
    const tradingPromises = tradingAgents.map(agent => executeAgent(agent, marketContext, marketData));
