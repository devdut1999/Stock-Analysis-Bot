 ...
 */
export function extractConsensus(analysis) {
    let bullishCount = 0;
    let bearishCount = 0;
    let neutralCount = 0;
 ...
 */
function checkTechnicalAlignment(signal, currentPrice, supportLevels, resistanceLevels) {
    if (signal === 'HOLD')
        return true;
    if (signal === 'BUY') {
        // Buy signal should be near support or after breakout above resistance
        const nearSupport = supportLevels.some(level => Math.abs((currentPrice - level) / level) < 0.03 // Within 3% of support
        );
    ...
    if (signal === 'SELL') {
        ...
        const nearResistance = resistanceLevels.some(level => Math.abs((currentPrice - level) / level) < 0.03 // Within 3% of resistance
        );
 ...
 */
function checkLiquidity(volume, minVolume, market, currentPrice) {
    if (market === 'INDIA') {
 ...
 */
function checkCircuitBreaker(currentPrice, previousClose, threshold = 5) {
 ...
 */
export function validateSignal(signal, marketData, analysis, config = DEFAULT_VALIDATION_CONFIG) {
    ...
    if (!consensusReached) {
        failureReasons.push(`Insufficient consensus: ${consensus.consensusStrength}% (required: ${config.minConsensusPercentage}%)`);
    }
    ...
    if (config.requireTechnicalAlignment && signal.signal !== 'HOLD') {
        ...
        technicalAlignment = checkTechnicalAlignment(signal.signal, marketData.price.currentPrice, supportLevels, resistanceLevels);
        if (!technicalAlignment) {
            failureReasons.push(`Entry not aligned with technical levels (${signal.signal} signal should be near support/resistance)`);
        }
    }
    // 3. Check risk limits
    ...
    if (signal.positionSizing) {
        if (signal.positionSizing.recommendedSize > config.maxPositionSize) {
            ...
            failureReasons.push(`Position size ${signal.positionSizing.recommendedSize}% exceeds limit of ${config.maxPositionSize}%`);
        }
        ...
        if (signal.stopLoss && signal.entryPrice) {
            ...
            if (riskPerTrade > config.maxRiskPerTrade) {
                riskWithinLimits = false;
                failureReasons.push(`Risk per trade ${riskPerTrade.toFixed(2)}% exceeds limit of ${config.maxRiskPerTrade}%`);
            }
        }
    }
    ...
    const sufficientLiquidity = checkLiquidity(marketData.price.volume, minVolumeForMarket, marketData.market, marketData.price.currentPrice);
    if (!sufficientLiquidity) {
        failureReasons.push(`Insufficient liquidity: ${marketData.price.volume.toLocaleString()} volume ` +
    ...
    if (marketData.market === 'INDIA' && !config.allowNearCircuitLimit) {
        circuitBreakerCheck = checkCircuitBreaker(marketData.price.currentPrice, marketData.price.previousClose, config.circuitLimitThreshold);
        if (!circuitBreakerCheck) {
            failureReasons.push(`Stock is within ${config.circuitLimitThreshold}% of circuit limit ` +
 ...
 */
export function getValidationEmoji(validation) {
    if (validation.allValidationsPassed)
        return '✅';
    if (validation.failureReasons.length === 1)
        return '⚠️';
    return '❌';
}
//# sourceMappingURL=signal-validator.js.map