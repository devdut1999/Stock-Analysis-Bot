 ...
 */
import { extractConsensus, validateSignal, formatValidationResult } from './signal-validator.js';
import { calculatePositionSize, calculateRiskMetrics, DEFAULT_PORTFOLIO_CONFIG } from './risk-manager.js';
/**
 * Extract price levels from synthesis text
 */
function extractPriceLevels(synthesisText, currentPrice) {
 ...
 */
function extractConviction(synthesisText) {
 ...
 */
function extractTimeHorizon(synthesisText) {
 ...
 */
function extractReasoning(analysis, marketData) {
 ...
 */
export function generateTradingSignal(marketData, analysis, portfolioConfig = DEFAULT_PORTFOLIO_CONFIG) {
    const errors = [];
    const warnings = [];
    try {
        // Extract consensus
        ...
        const { entry, stopLoss, takeProfit1, takeProfit2 } = extractPriceLevels(analysis.synthesis.analysis, marketData.price.currentPrice);
        // Extract time horizon
        const timeHorizon = extractTimeHorizon(analysis.synthesis.analysis);
        // Calculate position sizing
        const positionSizing = calculatePositionSize(conviction, entry, stopLoss, marketData, portfolioConfig);
        // Calculate risk metrics
        const riskMetrics = calculateRiskMetrics(entry, stopLoss, takeProfit1, marketData, portfolioConfig);
        // Extract reasoning
 ...
 */
export function formatTradingSignal(signal, includeValidation = true) {
    const lines = [];
