 ...
 */
export async function getPromoterHolding(symbol) {
 ...
 */
export function analyzePromoterHolding(holding) {
    const insights = [];
    const redFlags = [];
 ...
 */
export async function getPromoterPledgeTrend(symbol) {
    await rateLimiter.waitForSlot('nseIndia');
    try {
        const url = `${NSE_BASE_URL}/corporate-shareholding-pattern`;
        const response = await axios.get(url, {
 ...
 */
export function isPromoterPledgeIncreasing(trend) {
    if (trend.length < 2) {
        return {
            isIncreasing: false,
            changePercentage: 0,
