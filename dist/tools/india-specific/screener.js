 ...
 */
export async function getScreenerPromoterData(symbol) {
 ...
 */
export async function getScreenerFundamentals(symbol) {
    await rateLimiter.waitForSlot('screener');
    try {
        const url = `${SCREENER_BASE_URL}/company/${symbol}/`;
        const response = await axios.get(url, {
        ...
        $('li').each((_, element) => {
            ...
            if (text.includes('Dividend Yield:')) {
                ...
                if (match)
                    fundamentals.dividendYield = parseFloat(match[1]);
            }
            // ROCE
            if (text.includes('ROCE:')) {
                const match = text.match(/ROCE:\s*([\d.]+)%/);
                if (match)
                    fundamentals.roce = parseFloat(match[1]);
            }
            // ROE
            if (text.includes('ROE:')) {
                const match = text.match(/ROE:\s*([\d.]+)%/);
                if (match)
                    fundamentals.roe = parseFloat(match[1]);
            }
        });
 ...
 */
export async function getScreenerCompanyData(symbol) {
