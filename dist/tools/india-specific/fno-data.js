 ...
 */
export async function getFnOData(symbol) {
 ...
 */
function calculateMaxPain(records, currentPrice) {
    const strikes = [];
 ...
 */
function calculateAverageIV(records, currentPrice) {
    ...
    const atmRecord = records.find((r) => parseFloat(r.strikePrice) === atmStrike);
    if (!atmRecord)
        return 0;
 ...
 */
export function analyzeFnOData(fnoData, currentPrice) {
    const signals = [];
    // PCR analysis
 ...
 */
export async function isStockInFnO(symbol) {
    try {
...
//# sourceMappingURL=fno-data.js.map