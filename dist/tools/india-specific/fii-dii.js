 ...
 */
export async function getFIIDIIActivity(date) {
    ...
    try {
        ...
        const fiiData = data.find((item) => item.category === 'FII' || item.investorType === 'FII');
        // DII data (Domestic Institutional Investors)
        const diiData = data.find((item) => item.category === 'DII' || item.investorType === 'DII');
 ...
 */
export function analyzeFIIDIISentiment(trendData) {
...
//# sourceMappingURL=fii-dii.js.map