 ...
 */
export async function getYahooQuote(symbol, exchange) {
 ...
 */
export async function getYahooHistoricalData(symbol, exchange, days = 90) {
    await rateLimiter.waitForSlot('yahooFinance');
    try {
        const yahooSymbol = toYahooSymbol(symbol, exchange);
 ...
 */
export async function getYahooFundamentals(symbol, exchange) {
    await rateLimiter.waitForSlot('yahooFinance');
    try {
        const yahooSymbol = toYahooSymbol(symbol, exchange);
 ...
 */
export async function searchYahooStocks(query) {
    await rateLimiter.waitForSlot('yahooFinance');
    try {
 ...
 */
export async function getYahooOptionsChain(symbol, exchange) {
    await rateLimiter.waitForSlot('yahooFinance');
    try {
        const yahooSymbol = toYahooSymbol(symbol, exchange);
