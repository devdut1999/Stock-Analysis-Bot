...
import type { AggregatedStockData } from '../types/stock-data';

/**
 * Portfolio configuration
 */
export interface PortfolioConfig {
  totalCapital: number; // Total portfolio value
  maxPositionSize: number; // Max % per position (default: 20%)
  maxRiskPerTrade: number; // Max % risk per trade (default: 2%)
  maxPortfolioRisk: number; // Max total portfolio risk (default: 6%)
  riskFreeRate: number; // For Sharpe ratio calculation (default: 4%)
}
...
function calculateKellyPositionSize(
  winRate: number,  // 0-1 (e.g., 0.6 for 60%)
...
function calculateRiskBasedPositionSize(
  riskPerTrade: number,    // % of portfolio to risk
  stopLossPercent: number,  // % from entry to stop loss
 ...
 */
function calculateConvictionBasedSize(
...
export function calculatePositionSize(
  ...
  const riskBasedSize = calculateRiskBasedPositionSize(
  ...
  const convictionSize = calculateConvictionBasedSize(
    conviction,
    riskBasedSize,
    config.maxPositionSize
  );
  ...
  const kellySize = calculateKellyPositionSize(
    estimatedWinRate,
    stopLossPercent * 2, // Assume 2:1 reward/risk
    stopLossPercent,
    config.maxPositionSize
  );
  ...
  let scalingStrategy: 'all-at-once' | 'scale-in-2' | 'scale-in-3';
 ...
 */
function estimateVolatility(marketData: AggregatedStockData): number {
 ...
 */
function estimateWinRate(marketData: AggregatedStockData): number {
  let score = 0.5; // Base 50% win rate

  if (marketData.technicals.rsi !== null) {
    // Oversold = bullish = higher win rate
    if (marketData.technicals.rsi < 30) score += 0.1;
    if (marketData.technicals.rsi > 70) score -= 0.1;
  }

  if (marketData.technicals.macd) {
    // Positive histogram = bullish
    if (marketData.technicals.macd.histogram > 0) score += 0.05;
    else score -= 0.05;
  }
 ...
 */
export function formatPositionSizing(sizing: PositionSizing, currency: '₹' | '$' = '₹'): string {
  const lines: string[] = [];
